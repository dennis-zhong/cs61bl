...
public class Graph implements Iterable<Integer> {
    ...
    public List<Integer> shortestPath(int start, int stop) {
        Integer[] weights = new Integer[vertexCount];
        Integer[] predecesors = new Integer[vertexCount];
        for(int i = 0; i < vertexCount; i++) {
            weights[i] = null;
            predecesors[i] = null;
        }
        HashSet<Integer> visited = new HashSet<>();
        PriorityQueue<Integer> fringe = new PriorityQueue<>(Comparator.comparingInt(a -> weights[a]));
        fringe.add(start);
        weights[start] = 0;
